...
Ea=284.52
A=8.198*10**14
def ODEfun(Yfuncvec, V, UA,FA0,FI0,CpA,CpB,CT0,mc,Cpcoolant,CpC,To,CpI,Ea,A):
    Ta= Yfuncvec[0]
    T= Yfuncvec[1]
    X= Yfuncvec[2]
...
y0 = np.array([1250,1035,0]) # Initial values for the dependent variables

#%%
fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2)
fig.suptitle("""LEP-12-2c:Production of Acetic Anhydride (Co-Current)""", fontweight='bold', x = 0.19,y=0.97)
plt.subplots_adjust(left  = 0.48)
fig.subplots_adjust(wspace=0.25,hspace=0.3)
sol = odeint(ODEfun, y0, Vspan, (UA,FA0,FI0,CpA,CpB,CT0,mc,Cpcoolant,CpC,To,CpI,Ea,A))
Ta =sol[:, 0]
T = sol[:, 1]
X = sol[:, 2]
FT0=FI0+FA0;
...
ax_CpB = plt.axes([0.24, 0.68, 0.15, 0.015], facecolor=axcolor)
ax_CT0 = plt.axes([0.24, 0.64, 0.15, 0.015], facecolor=axcolor)
ax_CpC = plt.axes([0.24, 0.60, 0.15, 0.015], facecolor=axcolor)
ax_mc = plt.axes([0.24, 0.56, 0.15, 0.015], facecolor=axcolor)
ax_UA = plt.axes([0.24, 0.52, 0.15, 0.015], facecolor=axcolor)
...
sEa= Slider(ax_Ea, r'$E$ ($\frac{kJ}{mol}$)', 250, 330, valinit=284.5,valfmt='%1.1f')
...
resetax = plt.axes([0.26, 0.8, 0.09, 0.04])
