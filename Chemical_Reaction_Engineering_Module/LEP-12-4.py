...
def func(T,dH,A,Ea,T0,FM0,FI0,FB0,V,FA0,U,Ar,Ta):
    ...
    XEB=(sumCp*(T-T0)+((UA/FA0)*(T-Ta)))/(-(dH-7*(T-528)))
    return np.array([XMB, XEB])
    

#%%
fig, ax = plt.subplots()
fig.suptitle("""Example 12-4 CSTR with a Cooling Coil""", fontweight='bold', x = 0.15, y= 0.98)
plt.subplots_adjust(left  = 0.55)

sol = func(T,dH,A,Ea,T0,FM0,FI0,FB0,V,FA0,U,Ar,Ta)
XMB = sol[0, :]
XEB = sol[1, :]
...
ax.text(345, 0.25,
         ...
         '\n\n'
         r'$X_{EB}=\dfrac{\sum_{i}\theta_iC_{P_i} (T-T_{i0})+[UA (T-T_a)/F_{A0}]}{-\Delta H^\circ_{Rx}+7 (T-T_R)}$'
         , ha='left', wrap = True, fontsize=13,
        bbox=dict(facecolor='none', edgecolor='black', pad=13), fontweight='bold')
#%%
axcolor = 'black'
ax_dH = plt.axes([0.33, 0.8, 0.12, 0.02], facecolor=axcolor)
ax_A = plt.axes([0.33, 0.75, 0.12, 0.02], facecolor=axcolor)
ax_Ea = plt.axes([0.33, 0.7, 0.12, 0.02], facecolor=axcolor)
ax_T0 = plt.axes([0.33, 0.65, 0.12, 0.02], facecolor=axcolor)
ax_FM0 = plt.axes([0.33, 0.60, 0.12, 0.02], facecolor=axcolor)
ax_FI0 = plt.axes([0.33, 0.55, 0.12, 0.02], facecolor=axcolor)
ax_FB0 = plt.axes([0.33, 0.50, 0.12, 0.02], facecolor=axcolor)
ax_V = plt.axes([0.33, 0.45, 0.12, 0.02], facecolor=axcolor)
ax_FA0 = plt.axes([0.33, 0.4, 0.12, 0.02], facecolor=axcolor)
ax_U = plt.axes([0.33, 0.35, 0.12, 0.02], facecolor=axcolor)
ax_Ar = plt.axes([0.33, 0.3, 0.12, 0.02], facecolor=axcolor)
ax_Ta = plt.axes([0.33, 0.25, 0.12, 0.02], facecolor=axcolor)

sdH = Slider(ax_dH, r'$\Delta H^\circ_{Rx} (\frac{Btu}{lb-mol})$', -100000, -5000, valinit=-36400,valfmt='%1.0f')
sA = Slider(ax_A, r'$A (hr^{-1})$', 10**12, 100*10**12, valinit=16.96*10**12,valfmt='%1.2E')
...
sU= Slider(ax_U, r'$U (\frac{Btu}{hr.ft^{2}.^\circ F})$', 10, 500, valinit=100,valfmt='%1.0f')
sAr= Slider(ax_Ar, r'$A (ft^{2})$', 10, 500, valinit=40,valfmt='%1.1f')
sTa= Slider(ax_Ta, r'$T_{a} (^\circ R)$', 400, 700, valinit=545,valfmt='%1.0f')
