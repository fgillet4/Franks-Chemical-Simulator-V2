...
A1= 2.695*10**7
A2=1.106*10**9
def ODEfun(Yfuncvec, V,p,E1,E2,Cto,Cpco,mc,To,R,DH1b,DH2a,Tc,Cpa,Cpb,Cpc,Cpd,Ua,Ta0,A1,A2):
    Fa= Yfuncvec[0]
    Fb= Yfuncvec[1]
    Fc= Yfuncvec[2]
    Fd= Yfuncvec[3]
...
fig.suptitle("""LEP-12-7c:Complex Reactions with Heat Effects in a PFR (Constant Ta)""", fontweight='bold', x = 0.5,y=0.97)
fig.subplots_adjust(wspace=0.25,hspace=0.3)
sol = odeint(ODEfun, y0, Vspan, (p,E1,E2,Cto,Cpco,mc,To,R,DH1b,DH2a,Tc,Cpa,Cpb,Cpc,Cpd,Ua,Ta0,A1,A2))
Fa =sol[:, 0]
Fb = sol[:, 1]
Fc = sol[:, 2]
Fd = sol[:, 3]
...
ax1.text(-1.15, -1.25,'Differential Equations'
         ...
         '\n'
         r'$\dfrac{dT_a}{dV} = 0*\dfrac{Ua*(T - T_a)}{m_{C}* C_{P_{CO}}}$'
         '\n'
          'Explicit Equations'
                  '\n'
         r'$p=1$'
...
ax_Ta0 = plt.axes([0.32, 0.73, 0.2, 0.015], facecolor=axcolor)
...
sE2= Slider(ax_E2, r'$E_2$ ($\frac{cal}{mol}$)', 1000, 30000, valinit=12000,valfmt='%1.0f')
sCto = Slider(ax_Cto,r'$C_{T0}$ ($\frac{mol}{dm^3}$)', 0.05, 1, valinit= 0.2,valfmt='%1.2f')
sTa0 = Slider(ax_Ta0,r'$T_a$ ($K$)', 250, 400, valinit= 325,valfmt='%1.1f')
...
sUa= Slider(ax_Ua, r'Ua ($\frac{cal}{m^3.min.K}$)', 5, 500, valinit=80,valfmt='%1.1f')
