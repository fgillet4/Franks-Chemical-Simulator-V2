...
Ta0=315
A=1.073*10**11;
def ODEfun(Yfuncvec, V, Ca0,dH,Cpc,mc,UA,FT0,yA0,CpI,CpA,Ea,Ta0,A):
    Ta= Yfuncvec[0]
    T= Yfuncvec[1]
    X= Yfuncvec[2]
    thetaI=(1-yA0)/yA0;
    ...
    dTadV = 0*UA * (T - Ta) / (mc*Cpc); 
    dTdV = (ra * dH - (UA * (T - Ta))) / (sumcp * Fa0); 
    dXdV = 0 - (ra / Fa0);  
    return np.array([dTadV,dTdV,dXdV])

Vspan = np.linspace(0, 5, 1000) # Range for the independent variable
y0 = np.array([Ta0,305,0]) # Initial values for the dependent variables

#%%
fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2)
plt.subplots_adjust(left  = 0.43)
fig.subplots_adjust(wspace=0.25,hspace=0.3)
fig.suptitle("""LEP-12-1c: Butane Isomerization (Constant $T_a$)""", fontweight='bold', x = 0.18,y=0.97)

sol = odeint(ODEfun, y0, Vspan, (Ca0,dH,Cpc,mc,UA,FT0,yA0,CpI,CpA,Ea,Ta0,A))
Ta =sol[:, 0]
T = sol[:, 1]
X = sol[:, 2]
k = A*np.exp(-Ea/(8.31*T)); 
...
ax2.legend(['X','$X_e$'], loc='lower right')
ax2.set_ylim(0,1)
ax2.set_xlim(0,5)
ax2.grid()
...
#ax1.axis('off')
ax1.text(-10,80,'Note: While we used the expression k=$k_1$*exp(E/R*(1/$T_1$ - 1/$T_2$)) \n         in the textbook, in python we have to use k=A*exp(-E/RT) \n          in order to explore all the variables.',wrap = True, fontsize=13,
        bbox=dict(facecolor='none', edgecolor='red', pad=10))
ax1.text(-9.9, 130,'Differential Equations'
         '\n\n'
         r'$\dfrac{dT_a}{dV} = \dfrac{Ua*(T-T_a)}{m_c*C_{P_{c}}}*0$'
         '\n'
         ...
         '\n\n'
          ...
          r'$F_{T0} = 163\thinspace kmol/h$'
                  '\n'
         r'$K_c = 3.03*exp\left(\left(\dfrac{\Delta H_{Rx}}{8.31}\right)\left(\dfrac{1}{333} - \dfrac{1}{T}\right)\right)$'
                  '\n'
...
ax_UA = plt.axes([0.24, 0.70, 0.1, 0.015], facecolor=axcolor)
ax_yA0 = plt.axes([0.24, 0.66, 0.1, 0.015], facecolor=axcolor)
ax_Ea = plt.axes([0.24, 0.62, 0.1, 0.015], facecolor=axcolor)
ax_Ta0 = plt.axes([0.24, 0.58, 0.1, 0.015], facecolor=axcolor)
...
sTa0= Slider(ax_Ta0, r'T$_a$ ($K$)', 280, 400, valinit=315,valfmt='%1.0f')
