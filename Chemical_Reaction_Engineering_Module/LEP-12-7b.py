...
y0 = np.array([5, 10, 0, 0, 300, Ta0]) # Initial values for the dependent variables

#%%
fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2)
plt.subplots_adjust(left  = 0.32)
fig.suptitle("""LEP-12-7b:Complex Reactions with Heat Effects in a PFR (Counter-current)""", fontweight='bold', x = 0.5,y=0.98)
...
ax1.text(-1.15, -1.25,'Differential Equations'
        ...
        , ha='left', wrap = True, fontsize=10.5,
        bbox=dict(facecolor='none', edgecolor='black', pad=10.0), fontweight='bold')
ax1.text(0.9, 1.1,
         'Instructions: Guess a value of $T_a (T_{a0})$ at V=0 and see if it matches $T_a$=325 K at V=10 ${dm^3}$'
        , ha='left', wrap = True, fontsize=11, fontweight='bold',color='red')
axcolor = 'black'
...
ax_Ta0 = plt.axes([0.32, 0.61, 0.2, 0.015], facecolor=axcolor)
ax_Cpa = plt.axes([0.32, 0.58, 0.2, 0.015], facecolor=axcolor)
ax_Cpb = plt.axes([0.32, 0.55, 0.2, 0.015], facecolor=axcolor)
ax_Cpc = plt.axes([0.32, 0.52, 0.2, 0.015], facecolor=axcolor)
ax_Cpd = plt.axes([0.32, 0.49, 0.2, 0.015], facecolor=axcolor)

sE1 = Slider(ax_E1, r'$E_1$($\frac{cal}{mol}$)', 5000, 12000, valinit=8000,valfmt='%1.0f')
...
sTa0= Slider(ax_Ta0, '$T_{ao}$ (K)', 273, 1000, valinit=507,valfmt='%1.0f')
